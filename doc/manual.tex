...
Unison has to be built from source as we do not yet provide pre-compiled
packages.
%
...
Just go to the \texttt{src} directory and run:
%
\begin{command}
make build
\end{command}
...
system directory.
%
To install the binaries under the default directory \texttt{usr/local} just run:
%
\begin{command}
make install
\end{command}

The installation directory is specified by the Makefile variable \texttt{PREFIX}.
%
To install the binaries under an alternative directory \texttt{\$DIR} just run:
%
\begin{command}
...
The \texttt{uni} tool has several options and commands such as \texttt{run}.
%
