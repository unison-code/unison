...
parent i
  | i `elem`
      [ABS, ADCri, ADCrr, ADCrsi, ADCrsr, ADDSri, ADDSrr, ADDSrsi,
       ...
       VLDRD, VLDRD_fi, VLDRD_fi_demat_fi, VLDRD_fi_remat_fi,
       VLDRD_fi_source_fi, VLDRS, VLDRS_fi, VLDRS_fi_demat_fi,
       VLDRS_fi_remat_fi, VLDRS_fi_source_fi, VMAXNMD, VMAXNMNDf,
       VMAXNMNDh, VMAXNMNQf, VMAXNMNQh, VMAXNMS, VMAXfd, VMAXfq, VMAXhd,
       VMAXhq, VMAXsv16i8, VMAXsv2i32, VMAXsv4i16, VMAXsv4i32, VMAXsv8i16,
       ...
       T2ADDSrr, T2ADDSrs, T2ADDri, T2ADDri12, T2ADDri_fi,
       T2ADDri_fi_demat_fi, T2ADDri_fi_remat_fi, T2ADDri_fi_source_fi,
       T2ADDrr, T2ADDrs, T2ADR, T2ANDri, T2ANDrr, T2ANDrs, T2ASRri,
       T2ASRrr, T2B, T2BFC, T2BFI, T2BICri, T2BICrr, T2BICrs, T2BR_JT,
       T2BXJ, T2Bcc, T2CDP, T2CDP2, T2CLREX, T2CLZ, T2CMNri, T2CMNzrr,
       T2CMNzrs, T2CMPri, T2CMPrr, T2CMPrs, T2CPS1p, T2CPS2p, T2CPS3p,
       T2CRC32B, T2CRC32CB, T2CRC32CH, T2CRC32CW, T2CRC32H, T2CRC32W,
       T2DBG, T2DCPS1, T2DCPS2, T2DCPS3, T2DMB, T2DSB, T2EORri, T2EORrr,
       T2EORrs, T2HINT, T2HVC, T2ISB, T2IT, T2Int_eh_sjlj_setjmp,
       ...
       T2LDMDB, T2LDMDB_UPD, T2LDMIA, T2LDMIA_RET, T2LDMIA_UPD, T2LDRBT,
       T2LDRB_POST, T2LDRB_PRE, T2LDRBi12, T2LDRBi12_fi,
       ...
       T2LEApcrelJT_source, T2LSLri, T2LSLrr, T2LSRri, T2LSRrr, T2MCR,
       T2MCR2, T2MCRR, T2MCRR2, T2MLA, T2MLS, T2MOVCCasr, T2MOVCCi,
       T2MOVCCi16, T2MOVCCi32imm, T2MOVCClsl, T2MOVCClsr, T2MOVCCr,
       T2MOVCCror, T2MOVSsi, T2MOVSsr, T2MOVTi16, T2MOVTi16_ga_pcrel,
       T2MOV_ga_pcrel, T2MOVi, T2MOVi16, T2MOVi16_demat,
       T2MOVi16_ga_pcrel, T2MOVi16_remat, T2MOVi16_source, T2MOVi32imm,
       T2MOVi32imm_demat, T2MOVi32imm_remat, T2MOVi32imm_source,
       T2MOVi_demat, T2MOVi_remat, T2MOVi_source, T2MOVr, T2MOVsi,
       ...
       T2ORNrs, T2ORRri, T2ORRrr, T2ORRrs, T2PKHBT, T2PKHTB, T2PLDWi12,
       ...
       TMOVCCr_pseudo, TMOVSr, TMOVi8, TMOVi8s, TMOVi8s_demat,
       TMOVi8s_remat, TMOVi8s_source, TMOVr, TMUL, TMULz, TMVN, TORR,
       TPICADD, TPOP, TPOP2_r4_11, TPOP2_r4_7, TPOP_RET, TPOP_r4_7,
    ...
    = Nothing
  | i `elem`
      [FMSTAT_cpsr, FMSTAT_cpsr_demat, FMSTAT_cpsr_remat,
       FMSTAT_cpsr_source]
    = Just FMSTAT
  | i `elem`
      [VLDRD_cpi, VLDRD_cpi_demat_cpi, VLDRD_cpi_remat_cpi,
       VLDRD_cpi_source_cpi]
    = Just VLDRD
  | i `elem`
      [VLDRS_cpi, VLDRS_cpi_demat_cpi, VLDRS_cpi_remat_cpi,
       VLDRS_cpi_source_cpi]
    = Just VLDRS
  ...
  | i `elem`
      [T2LEApcrel_cpi, T2LEApcrel_cpi_demat_cpi,
       T2LEApcrel_cpi_remat_cpi, T2LEApcrel_cpi_source_cpi]
    = Just T2LEApcrel
